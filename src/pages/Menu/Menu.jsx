...
export const Menu = () => {
  ...
  const fetchCategoryData = async () => {
    const url = `https://www.themealdb.com/api/json/v1/1/filter.php?c=${selectedCat}`;
    const repsonse = await fetch(url);
    const result = await repsonse?.json();
    setListOfData(result?.meals);
  };
  const categories = [
    {
      idCategory: "1",
  ...
  return (
    <section className="mt-3 text-black px-5 flex flex-col justify-center flex-wrap">
      ...
      <section className="flex justify-center items-center my-5">
        <select
          ...
          name="categories"
          className="border border-solid border-gray-500 w-40"
        >
          {categories?.map((val) => (
            <option value={val?.strCategory} key={val?.strCategory}>
              {val?.strCategory}
            </option>
          ))}
        </select>
